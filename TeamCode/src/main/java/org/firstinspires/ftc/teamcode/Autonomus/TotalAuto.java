...
public class TotalAuto extends LinearOpMode {
    private VerticalSlideRR verticalSlideRR = new VerticalSlideRR(hardwareMap);
    private HorizontalSlideRR horizontalSlideRR = new HorizontalSlideRR(hardwareMap, telemetry);
    private VerticalGrabberRR verticalGrabberRR = new VerticalGrabberRR(telemetry, hardwareMap);
    private VerticalWristRR verticalWristRR = new VerticalWristRR(telemetry, hardwareMap);
    private HorizontalWristRR horizontalWristRR = new HorizontalWristRR(telemetry, hardwareMap);
    private HorizontalGrabberRR  horizontalGrabberRR = new HorizontalGrabberRR(telemetry, hardwareMap);
    private HorizontalRollRR horizontalRollRR = new HorizontalRollRR(telemetry, hardwareMap);
    private ConfigPose configPose = new ConfigPose();
    ...
    boolean pickHangPost = false;

    int hangSide = 1;
    int butterSide = 1;
    int parkSide = 1;
    int waitHuman = 0;
    //make a class for Vertical Slide
    ...
    //add a class for Horizontal Wrist
    ...
    //add a class for Horizontal Roll
    ...
    //make a class that update the pose
    public class ConfigPose{
        public class PoseUpdate implements Action{
            @Override
            public boolean run(@NonNull TelemetryPacket packet){
                initialPose = new Pose2d(drive.pose.position.x,drive.pose.position.y,drive.pose.heading.toDouble());
                return false;
            }
        }
        public Action poseUpdate(){
    ...
    public void runOpMode() throws InterruptedException {
        ...
        TrajectoryActionBuilder hang = drive.actionBuilder(initialPose)
                ...
                .afterTime(0,configPose.poseUpdate());
        ...
        if (gamepad1.square && !gamepad1.circle) {
            ...
            while (!gamepad1.right_bumper) {
                ...
                telemetry.addLine("cross-HangPost" + pickHangPost);
                if (gamepad1.square && !gamepad1.triangle && !gamepad1.circle && !gamepad1.cross && !gamepad1.left_bumper) {
                    pickHang = true;
                    hangSide = -1;
                }
                if (!gamepad1.square && gamepad1.triangle && !gamepad1.circle && !gamepad1.cross && !gamepad1.left_bumper) {
        ...
        if (!gamepad1.square && gamepad1.circle) {
            ...
            while (!gamepad1.right_bumper) {
                telemetry.clear();
                telemetry.addLine("square-Hang: " + pickHang);
                telemetry.addLine("triangle-Human/hangPost: " + pickHuman);
                telemetry.addLine("circle-Park" + pickPark);
        ...
        if (pickLeftSide){
            if(pickHang){
                new SequentialAction(hang.build());
            }
            if(pickButter) {
                new SequentialAction(butterLeft.build());
            }
        }
        if (pickRightSide){
            if(pickHang){
            ...
            if(pickButter){
                new SequentialAction(butterRight.build());
            }
            if(pickHuman){
                new SequentialAction();
            }
            if(pickHangPost){
                new SequentialAction(hangPost.build());
            }
            //run the chosen action blocking
            new SequentialAction(wait.build());
        }
    }
}
